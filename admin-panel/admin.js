// admin.js — KFS Admin Panel (served only to authenticated admins via /admin-assets/)
// This file is NEVER served to unauthenticated visitors.
...
// ── openMovie / openBlog / navigate stubs for admin panel ─────────────────────
// The full implementations live in public-app.js (loaded on the public site).
// On the admin panel these open the public page in a new tab (for preview) or
// redirect to /admin for the 'admin' case.
function openMovie(id) {
  if (!id) return;
  window.open('/films/' + id, '_blank', 'noopener');
}
function openBlog(id) {
  if (!id) return;
  window.open('/blog/' + id, '_blank', 'noopener');
}
// navigate() on the admin panel: 'admin' stays on page, public pages open in new tab.
if (typeof navigate !== 'function') {
  window.navigate = function(page) {
    if (page === 'admin') { return; } // already on admin page
    if (page === 'home') { window.open('/', '_blank', 'noopener'); return; }
...
async function loadPaymentAnalytics() {
  // Populate Google Sheet link from server (ID lives in env var, never hardcoded in HTML)
  try {
    const sheetRes = await fetch('/api/admin/sheet-link', { headers: { Authorization: 'Bearer ' + adminToken } });
    const sheetData = await sheetRes.json();
    const sheetLink = document.getElementById('pa-sheets-link');
    if (sheetLink && sheetData.url) {
      sheetLink.href = sheetData.url;
      sheetLink.style.display = 'flex';
    }
  } catch(e) { /* non-critical — sheet link just stays hidden */ }
...
function _evalDataHandler(el, expr, eventObj) {
  ...
  for (const part of parts) {
    ...
    // Special: self-assignment like "this.value=this.value.replace(/[^0-9 ]/g,'')"
    const selfAssignMatch = part.match(/^this\.value=this\.value\.replace\((.+)\)$/);
    if (selfAssignMatch) {
      try {
        const fn = new Function('v', 'return v.replace(' + selfAssignMatch[1] + ')');
