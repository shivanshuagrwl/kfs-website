...
function openEventModal(ev=null) {
  ...
  const locLinkEl = document.getElementById('event-location-link');
  if (locLinkEl) locLinkEl.value = ev?.location_link||'';
  const waCheckEl = document.getElementById('event-show-whatsapp');
  const waLinkEl = document.getElementById('event-whatsapp-link');
  if (waCheckEl) waCheckEl.checked = !!(ev && ev.whatsapp_group_link);
  if (waLinkEl) waLinkEl.value = ev?.whatsapp_group_link || '';
...
async function saveEvent() {
  ...
  const locLinkInput = document.getElementById('event-location-link');
  if (locLinkInput) fd.append('location_link', locLinkInput.value);
  const waChecked = document.getElementById('event-show-whatsapp')?.checked;
      ...
      (async () => {
        try {
          ...
          if (ev) {
            ...
            setTimeout(() => {
              openEventForm(ev.id, ev.title, ev.event_date || '', ev.event_time || '', ev.location || '', ev.location_link || '', ev.whatsapp_group_link || '');
            }, 500);
            return;
          }
        } catch (e) {}
...
function openEventFormById(eventId) {
  ...
  if (!e) return;
  openEventForm(e.id, e.title, e.event_date || '', e.event_time || '', e.location || '', e.location_link || '', e.whatsapp_group_link || '');
}
...
async function openEventForm(eventId, eventTitle, eventDate='', eventTime='', eventLocation='', eventLocationLink='', eventWhatsappLink='') {
  ...
  window._rfEventLocationLink = eventLocationLink;
  window._rfEventWhatsappLink = eventWhatsappLink;
  _rfForm = null;
  _rfImageFiles = {};
  _rfSections = [];
