...
function openEventFormById(eventId) {
  ...
  if (!e) return;
  openEventForm(e.id, e.title, e.event_date || '', e.event_time || '', e.location || '', e.location_link || '');
}
...
// Reads from /api/strand/* (no auth). Interactions nudge to /Social-Strand.
// ═══════════════════════════════════════════════════════════════════════════
...
// Note: Escape-key handling and tag-filter debounce are wired above
// (keydown listener near pswCloseDetail, and inside loadStudio() respectively).
// Only the Load More button needs wiring here.
